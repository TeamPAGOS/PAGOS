...
# generate PAGOSUnits like mole_He, mole_Ne, ccSTP_He, gram_SF6, ...
for pattern in PAGOSDimPatterns:
    for gas in gases_info:
        ...
        for target_pattern in PAGOSDimPatterns:
            if target_pattern != pattern:
                ...
                transf_func_str = GenericPAGOSTransformations[_id]
                transf_func_name = (
                    transf_func_str.split("[GAS]")[0].removeprefix("def ")
                    + gdict["name"]
                )
                # rename the generic function with the specific gas
                ...
                # create association between function and hash key in PAGOSTransformations
                target_dimension = target_pattern[0] + gdict["name"] + "]"
                id = hash((dimension, target_dimension))
                exec(
                    f"PAGOSTransformations[id] = ('{dimension}', '{target_dimension}', {transf_func_name})",
                    globals(),
                    locals(),
                )
    ...
    aliases = [pattern[i] + "_gas" for i in range(2, len(pattern))] + [
        pattern[i] + "_g" for i in range(2, len(pattern))
    ]
    PAGOSUnits[baseunit] = reduce(
        add, (" = " + a for a in aliases), f"{baseunit} = {dimension}"
    )
