...
from magicgui.widgets import Table, Widget, Container, MainWindow, Select, FunctionGui, Dialog, Label, RadioButtons, TupleEdit
from magicgui.application import use_app
from pathlib import Path
from typing import Annotated, Literal, Iterable
from collections.abc import Sequence, Set
from qtpy.QtWidgets import QFileDialog, QAbstractItemView, QDialogButtonBox, QDialog, QPushButton
from inspect import signature, isfunction, getmodule, getsource
import pandas as pd
import re
...
# globals
list_of_builtin_models = [func.__name__ for func in pagos_bms.__dict__.values() if isfunction(func) and getmodule(func) == pagos_bms]
...
class Main():
    def __init__(self):
        ...
        self.maincontainer = MainWindow(
            widgets = [self.left, self.right], layout='horizontal', labels=False, name='PAGOS'
        )

        # some individual settings that couldn't be done inside magicgui
        try:
            self.left.modelfield.native.setFontFamily('mono')
            self.left.modelfield.value = self.left.modelfield.value # <- refresh so that the mono font is applied
        except:
            print('WARNING: Could not set font of code field, keeping default.')
        self.left.modelfield.native.setTabStopDistance(35.0)
        self.right.datatable.native.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers)
        ...
        #self.left.modelfield.changed.connect(self.code_changed) # TODO why does this not work?
        self.left.modelfield.native.textChanged.connect(self.code_changed)
        self.right.select_tracers.clicked.connect(lambda: self.select_tr_or_op(mode=1)) # TODO: can this cause memory leakage? see https://stackoverflow.com/questions/61871629/memory-profiler-while-using-lambda-expression-to-connect-slots
        self.right.select_other_params.clicked.connect(lambda: self.select_tr_or_op(mode=2)) # TODO: see above

        # menu
        self.maincontainer.create_menu_item('File', 'Import data', callback=self.openfile, shortcut='ctrl+o')
    ...
    def openfile(self):
        ...
        imported_data_path:str = use_app().get_obj('show_file_dialog')(mode='r', caption='Import data')
    ...
    def code_changed(self):
        if self.current_selected_model != custom_model_name_placeholder and not self.suppress_selected_model_change:
        ...
        if self.current_selected_model == custom_model_name_placeholder:
            self.saved_custom_model_code = self.left.modelfield.value
    ...
    def select_tr_or_op(self, mode):
        ...
        which_errors = set(tracer for tracer in selected_tracers if re.search(errorpattern, tracer))
        which_units = set(tracer for tracer in selected_tracers if re.search(unitpattern, tracer))
        if which_errors.intersection(which_units):
            ...
            print('CONFLICT: PAGOS could not parse which columns contain error data and which contain unit data!')
        else:
            tr_or_op_dict = {k:{'err':[], 'unit':[]} for k in selected_tracers.difference(which_errors, which_units)}
            
            for elt_i in tr_or_op_dict:
                for er_j in which_errors:
                    if re.search(rf"\b{re.escape(elt_i)}\b", er_j): tr_or_op_dict[elt_i]['err'].append(er_j)
                for un_j in which_units:
                    if re.search(rf"\b{re.escape(elt_i)}\b", un_j): tr_or_op_dict[elt_i]['unit'].append(un_j)
            ...
            for elt_i in tr_or_op_dict:
                ...
                if len(err) > 0 and manselect or len(err) > 1:
                    @magic_factory(auto_call=True, rbs={'widget_type':'RadioButtons', 'choices':err + ['none of the above']}, labels=False, tracername={'visible':False})
                    def err_radio(rbs=err[0], tracername=elt_i): return ('err', tracername)
                    eu_dialog_widgets.append(Label(value='Which of these is an error on %s?' % elt_i))
                    eu_dialog_widgets.append(err_radio())
                    n_err_widgs += 1
                # if manual unit selection is necessary
                if len(unit) > 0 and manselect or len(unit) > 1:
                    @magic_factory(auto_call=True, rbs={'widget_type':'RadioButtons', 'choices':unit + ['none of the above']}, labels=False, tracername={'visible':False})
                    def unit_radio(rbs=unit[0], tracername=elt_i): return ('unit', tracername)
                    eu_dialog_widgets.append(Label(value='Which of these is a unit on %s?' % elt_i))
            ...
            def finished_select_tracers():
                ...
                def raise_mode_error(): raise ValueError('Something has gone terribly wrong - mode argument to select_tr_or_op should only be 1 or 2, not %i' % mode)
                ...
                for q in tr_or_op_dict:
                    ...
                    if len(unit_entry:=tr_or_op_dict[q]['unit']) == 0:
                        ...
                        if unit_entry[0] == 'none of the above':
                            ...
                            units = self.right.datatable.to_dict('series')[unit_entry[0]]
                
                    if mode == 1:
                        self.used_tracers[q] = {'data':data, 'errs':errs, 'units':units}
                    elif mode == 2:
                        self.used_other_params[q] = {'data':data, 'errs':errs, 'units':units}
                    else:
                        raise_mode_error()
                
                # setting widgets
                isman = lambda x: type(x) == str and x == 'man' # this construction avoids "ValueError: The truth value of a Series is ambiguous" from Pandas.
                manual_errs = [q for q in self.used_tracers if isman(self.used_tracers[q]['errs'])] + [q for q in self.used_other_params if isman(self.used_other_params[q]['errs'])]
                manual_units = [q for q in self.used_tracers if isman(self.used_tracers[q]['units'])] + [q for q in self.used_other_params if isman(self.used_other_params[q]['units'])]
                ...
                # set text widget
                self.right.text_over_tracers1.value = 'Quantities taking errors from data table: ' + ', '.join([elt for elt in self.used_tracers if not isman(self.used_tracers[elt]['errs'])] + [elt for elt in self.used_other_params if not isman(self.used_other_params[elt]['errs'])])
                self.right.text_over_tracers2.value = 'Quantities taking units from data table: ' + ', '.join([elt for elt in self.used_tracers if not isman(self.used_tracers[elt]['units'])] + [elt for elt in self.used_other_params if not isman(self.used_other_params[elt]['units'])])
            ...
            if n_err_widgs == 0 and n_unit_widgs == 0:
                ...
                eu_msg = Label(value=f'Some columns you selected seem like they might be {textappend1} on tracer measurements{textappend2}. Please select those which are:')
                eu_dialog_widgets.insert(0, eu_msg)
                eu_dialog = Dialog(widgets=eu_dialog_widgets, labels=False, scrollable=True)
    ...
    @magic_factory(
        ...
        auto_call=True,
        labels=True,
        filename={'widget_type':'Label', 'value':'Add some data to get started', 'label':'Sample name'},
        modelselect={'widget_type':'ComboBox', 'choices':['Custom...'] + list_of_builtin_models, 'value':custom_model_name_placeholder, 'visible':False},
        select_to_fit={'widget_type':'Select', 'visible':False},
        modelfield={'widget_type':'TextEdit', 'visible':False, 'value':custom_model_code_placeholder},
    ...
    @magic_factory(
        ...
        auto_call=True,
        labels=False,
        man_eu={'widget_type':'CheckBox', 'text':'Manually select all units/errors', 'value':False},
        select_tracers={'widget_type':'PushButton', 'text':'Select tracers', 'tooltip':'Select columns in the data table below and then click here to set tracers you\'ll use in the model.'},
        select_other_params={'widget_type':'PushButton', 'text':'Select other parameters', 'tooltip':'Select columns in the data table below and then click here to set non-tracer parameters you\'ll use in the model.'},
        text_over_tracers1={'widget_type':'Label', 'value':'Quantities taking errors from data table:'},
        text_over_tracers2={'widget_type':'Label', 'value':'Quantities taking units from data table:'},
