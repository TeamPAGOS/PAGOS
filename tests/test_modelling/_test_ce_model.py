...
CEModel = GasExchangeModel(ce, ['degC', 'permille', 'atm', 'ccSTP_g/g_w', ''], 'ccSTP_g/g_w')

def test_ce_model_with_preset_data():
    CE_fit = CEModel.fit(preset_data, ['T', 'A', 'F'], [15, 5e-5, 0.5], NOBLEGASES, tqdm_bar=True)
    CE_fit = CE_fit.map(lambda x: snv(x)) # convert to nominal values
    np.testing.assert_allclose(CE_fit, preset_data[['T', 'A', 'F']], rtol=1e-2)


def test_ce_model_with_single_preset_data():
    obs, errs, units, ops = preset_data[NOBLEGASES].iloc[4], preset_data[[ng + ' err' for ng in NOBLEGASES]].iloc[4], 'ccSTP_g/g_w', preset_data[['S', 'p']].iloc[4]
