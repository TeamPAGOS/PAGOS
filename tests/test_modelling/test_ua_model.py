...
UAModel = GasExchangeModel(ua, ['degC', 'permille', 'atm', 'ccSTP_g/g_w'], 'ccSTP_g/g_w')

def test_ua_model_with_preset_data():
    UA_fit = UAModel.fit(preset_data, ['T', 'S', 'A'], [15, 25, 5e-5], NOBLEGASES, tqdm_bar=False)
    UA_fit = UA_fit.map(lambda x: x.nominal_value) # convert to nominal values
    np.testing.assert_allclose(UA_fit, preset_data[['T', 'S', 'A']], rtol=1e-2)

def test_ua_model_with_single_preset_data():
    obs, errs, units, ops = preset_data[NOBLEGASES].iloc[4], preset_data[[ng + ' err' for ng in NOBLEGASES]].iloc[4], 'ccSTP_g/g_w', preset_data[['p']].iloc[4]
