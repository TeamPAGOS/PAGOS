...
PRModel = GasExchangeModel(pr, ['degC', 'permille', 'atm', 'ccSTP_g/g_w', '', ''], 'ccSTP_g/g_w')

def test_pr_model_with_preset_data():
    PR_fit = PRModel.fit(preset_data, ['T', 'A', 'FPR'], [15, 5e-5, 0.5], NOBLEGASES, tqdm_bar=False)
    PR_fit = PR_fit.map(lambda x: snv(x)) # convert to nominal values
    np.testing.assert_allclose(PR_fit, preset_data[['T', 'A', 'FPR']], rtol=1e-2)

def test_pr_model_with_single_preset_data():
    obs, errs, units, ops = preset_data[NOBLEGASES].iloc[4], preset_data[[ng + ' err' for ng in NOBLEGASES]].iloc[4], 'ccSTP_g/g_w', preset_data[['S', 'p', 'beta']].iloc[4]
