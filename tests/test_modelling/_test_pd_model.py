...
PDModel = GasExchangeModel(_pd, ['degC', 'permille', 'atm', 'ccSTP_g/g_w', '', ''], 'ccSTP_g/g_w')

def test_pd_model_with_preset_data():
    PD_fit = PDModel.fit(preset_data, ['T', 'A', 'FPD'], [15, 5e-5, 0.5], NOBLEGASES, tqdm_bar=False)
    PD_fit = PD_fit.map(lambda x: snv(x)) # convert to nominal values
    np.testing.assert_allclose(PD_fit, preset_data[['T', 'A', 'FPD']], rtol=1e-2)

def test_pd_model_with_single_preset_data():
    obs, errs, units, ops = preset_data[NOBLEGASES].iloc[0].to_list(), preset_data[[ng + ' err' for ng in NOBLEGASES]].iloc[0].to_list(), 'ccSTP_g/g_w', preset_data[['S', 'p', 'beta']].iloc[0].to_list()
